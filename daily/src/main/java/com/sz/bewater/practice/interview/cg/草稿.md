...
分布式事务解决方案
1.XA模式   TM RM TC
TM事务管理器  作用是开启全局事务 管理各分支事务的状态   通常是我们开启全局事务的一方作为TM
RM  资源管理器 通常指的是各节点的本地事务
TC 事务协调器  作用是分支事务的注册点  与各分支事务保持通信  决定各分支事务的提交或者回滚
流程：
开启全局事务:
TM 向 TC 发起请求，开启一个新的全局事务。
分支事务注册:
各个 RM 向 TC 注册其分支事务，加入到全局事务中。
准备阶段（Prepare Phase）:
TM 向各个 RM 发起 prepare 指令，各个分支事务执行本地事务，但不提交，进入准备状态。
各个 RM 向 TC 汇报其准备状态（准备就绪或未准备好）。
检查准备状态:
TC 收集所有分支事务的准备状态。
如果有任何一个分支事务未准备好，则全局事务结束，TC 向所有 RM 发出回滚指令，所有分支事务回滚。
如果所有分支事务都准备就绪，进入提交阶段。
提交阶段（Commit Phase）:
TM 向 TC 发起提交全局事务的请求。
TC 检查所有分支事务的执行状态。
如果所有分支事务都成功，TC 向所有 RM 发出提交指令，各个分支事务提交本地事务。
如果有任何一个分支事务执行失败，TC 向所有 RM 发出回滚指令，各个分支事务回滚本地事务。
2.AT模式  seata的默认模式  各分支事务在一阶段可以独立提交  在二阶段做补偿  由TC根据各分支事务状态来提交或者回滚全局事务  相较于XA模式的优点  1.减少了对数据库资源的持有时间  2锁的粒度更小(行级锁)
3.TCC模式  本质上还是两阶段提交  try:对所需资源的提前占用  confirm:确认阶段  try全部成功则执行  cancel:回滚阶段  try有不成功的则执行  优点:相较于XA、AT模式强依赖于数据库  TCC由代码控制不依赖于数据库 自由度可定制度更高  缺点：需要程序员编写try、confirm、cancel三个方法  对代码的侵入性较高  
三阶段提交 相对于二阶段 加了一个canCommit阶段  假如有的分支事务 回复失败  则没必要接着走后续的两阶段流程了 提高了效率
...
CompletableFuture
1.runAsync 用于异步执行一个不返回结果的任务。
2.supplyAsync 用于异步执行一个返回结果的任务。
3.anyOf 用于等待多个 CompletableFuture 中的任意一个完成。
4.allOf 用于等待所有 CompletableFuture 都完成。
5.thenApply 用于在 CompletableFuture 完成后对结果进行转换。
6.thenAccept 用于在 CompletableFuture 完成后对结果进行消费。
7.thenRun 用于在 CompletableFuture 完成后执行一个动作，但不使用结果。
8.whenComplete 用于在 CompletableFuture 完成后执行一个动作，能够访问到结果和异常。
9.handle 用于在 CompletableFuture 完成后对结果或异常进行处理，并返回一个新的结果。
thenApply、thenAccept 都可以原future进行消费 但是thenApply可以返回一个新的带返回值的CompletableFuture而thenAccept不可以  thenApply作用是转换  thenAccept作用是消费
thenRun 不依赖于原future的返回值 强调的是原future完成后要做的事
whenComplete和  handle 都会返回一个新的CompletableFuture  但是whenComplete返回的新的CompletableFuture和传进来的future返回值必须相同  类似于thenAccpet
而handle可以返回与传进来的future返回值不同的CompletableFuture   类似于thenApply

-----------------------------------------------------Java基础-----------------------------------------------------
单例模式：懒汉模式（存在线程安全 直接加锁粒度太大）、饿汉模式（JVM保证线程安全 但是造成资源浪费）、DCL解决懒汉模式线程安全（双端检锁 降低锁粒度 但是存在锁和volatile 一定程度上损耗性能且代码较复杂 不如静态内部类）、静态内部类（最佳 静态内部类实现延迟加载、类加载机制JVM保证线程安全）
选用那种模式：看场景
1.//懒汉模式  需要时再创建对象
...
3.//dcl 加锁前后进行判断  double check  减小锁的粒度提高性能
...
---------------------------------------mq---------------------------------------------------------------
rocketmq事务消息 可靠消息实现最终一致性
事务发起者 发送half消息给rocketmq  mq回复接受成功给事务发起者  事务发起者执行本地事务 执行完本地事务 发送commit or rollback给mq 假如是commit mq会将half消息状态改为可消费  消费者就可以消息消息  假如是rollback mq就会将half消息删除    过程中注意幂等（执行本地事务幂等   消费端消费幂等）   half消息包含全局的事务id和业务消息
只要事务发起者执行完本地事务  消息就必须被消费者消费掉 （重复投递 人工处理）


RocketMQ实现顺序消息
RocketMQ 实现顺序消息分为全局有序和局部有序两种情况。
全局有序实现方式相对较为严格，要求生产者、MessageQueue 和消费者一一对应。即只有一个生产者，且该生产者只能向一个 MessageQueue 发送消息，同时只有一个消费者从这个唯一的 MessageQueue 中消费消息。这种方式能确保整个 Topic 内的消息严格按照先进先出的顺序进行发布和消费，但由于其限制了并发处理，对性能可能产生较大影响，因此在实际应用中较少使用。
局部有序则相对灵活。生产者需要使用 MessageQueueSelector 按照一定规则向队列投递消息，例如可以根据订单号等业务标识进行处理，将相关消息发送到同一个 MessageQueue 中。消费者则需要使用 MessageListenerOrderly 这个监听器来消费消息，并且不能采用并发消费的方式。这样可以保证在局部范围内，例如同一个订单的相关消息能够按照发送顺序进行消费。
在实际业务场景中，大多数情况下只需要保证局部有序即可满足需求。例如电商订单场景中，只要保证一个订单的所有消息有序，不同订单之间的消息顺序相对不那么重要。全局有序通常面临性能和扩展性的问题，而局部有序通过合理的设计和配置，能够在保证关键消息顺序的同时，提高系统的处理效率和灵活性。

RocketMQ
集群消费：一个消息只能被一个消费者消费（默认）  广播消费
发送：同步发送（安全但阻塞）  异步发送（有回调 相对安全 失败可重试） 单向发送（发了就不管）
延时消息： 提供了18个等级的延时 也提供了自定义延时时间
顺序消息：  全局有序 相当于弄成了一对一且消费单线程   部分有序：messageQueueSelector（生产者 按照规则往对应的队列发消息）+messageListenerOrderly（消费者）  消费者不能并发消费
事务消息： 实现RocketLocalTransactionalListener（执行本地事务+回查方法 需自己写代码实现） half消息——》执行本地事务——》返回状态——》成功则可消费失败则回滚未返回则回查（15次 超过则过期删除）——》消费者消费（一定得消费掉 最终一致性）
消息过滤：根据tag、sql
批量消息： 多个消息整合成集合 producer.send(messages)  减少网络IO提高吞吐量  但是单次不能超过4M  不适合批量、延迟、事务

Rocketmq的持久化机制
commitLog+consumerQueue+indexFile
commitLog:是消息真正存储的地方  一个commitLog大小通常是1G commitLog采用的是顺序写的方式 没有磁盘磁头寻址带来的性能消耗 写的性能很高  而且磁盘有预读的功能 每次都会读取若干个数据页到缓存中 又由于commitLog是顺序写 所以预读出来的数据就比较多 提高了读的性能  commitLog是一个个consumerQueue 对应一个个messageQueue
又由于messageQueue也可以在不同的broker上 使得并发写的效率也很高
consumerQueue: 可以理解成commitlog的索引文件 consumerQueue记录着commitLogOffer通过这个可以快速定位消息位于哪个commitLog文件中 consumerQueue还记录着tag的相关信息如hashCode 使得通过tag查询消息速度也变得很快
indexFile: 当给message设置key的时候 rockermq就会帮我们维护对应的indexFile文件 通过key可以查找到对应消息位于哪一个commitLog文件中
consumerQueue和indexFile都可以视为是rocketmq的索引文件

同步刷盘和异步刷盘
同步刷盘保证消息持久化到磁盘后才给生产者返回 ACK，确保消息可靠但可能影响性能；
异步刷盘在消息写入 pageCache 就返回 ACK 给生产者，采用异步线程刷盘，提高性能和吞吐但可能存在消息丢失风险。


kafka
攒一波再发送：生产者发生消息一般都是异步发送，生产者发送一条消息并不会立马到达broker 而是现在生产者客户端先缓存起来  等到了一定数量（有俩配置batch.size 默认是16kb  linger.ms消息间隔 俩条件满足其中之一就会发送）才会发送给broker 减少了网络IO提高了吞吐(损失了消息的及时性)
而rocketmq是pull+push(本质上也是pull 对pull做了封装 是一种长轮训的pull模式 默认15s 当长轮训期间有消息过来就会立马发送给消费者 及时性比kafka就要好了)两种模式